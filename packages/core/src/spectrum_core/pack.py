...
def _is_skipped_dir(path: Path) -> bool:
    ...
    return name in SKIPPED_DIR_NAMES or suffix in SKIPPED_DIR_SUFFIXES


def _is_encodable_file(path: Path, *, include_all: bool) -> bool:
    suffix = path.suffix.lower()
    if suffix in {".spec", ".specpack"}:
        return False
    if suffix in SKIPPED_FILE_EXTENSIONS:
        return False
    return include_all or suffix in SUPPORTED_EXTENSIONS


def iter_source_files(root: str | Path, *, include_all: bool = False) -> Iterable[Path]:
    root_path = Path(root).expanduser()
    if root_path.is_file():
        if _is_encodable_file(root_path, include_all=include_all):
            yield root_path
        return
    ...
    for dirpath, dirnames, filenames in os.walk(root_path, topdown=True, onerror=ignore_walk_error, followlinks=False):
        current = Path(dirpath)
        dirnames[:] = sorted(name for name in dirnames if not _is_skipped_dir(current / name))
        for filename in sorted(filenames):
            ...
            try:
                ...
                continue
            if _is_encodable_file(path, include_all=include_all):
                yield path
